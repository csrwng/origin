...
func TestSTICreateBuildPod(t *testing.T) {
	...
	if e := container.Env[0]; e.Name != "BUILD_TAG" && e.Value != expected.Config.ImageTag {
		t.Errorf("Expected %s, got %s:%s!", expected.Config.ImageTag, e.Name, e.Value)
	}
	...
	if e := container.Env[2]; e.Name != "SOURCE_REF" && e.Value != expected.Config.SourceURI {
		t.Errorf("Expected %s got %s:%s!", expected.Config.SourceURI, e.Name, e.Value)
	}
	if e := container.Env[3]; e.Name != "BUILDER_IMAGE" && e.Value != expected.Config.BuilderImage {
		t.Errorf("Expected %s, got %s:%s!", expected.Config.BuilderImage, e.Name, e.Value)
	}
}
